...
## 🔴 Grundsatz 1: Native Paper API (gilt ausnahmslos für jede Änderung)
...
## 🔴 Grundsatz 2: 100 % natives Kotlin (gilt ausnahmslos für jede Änderung)

Dieses Projekt ist **vollständig in Kotlin** geschrieben und wird mit **Gradle** gebaut.

**MANDATORY:** Jede Erweiterung, jedes Refactoring, jeder Befehl, Listener, jede GUI und jede
Korrektur **MUSS in nativem Kotlin** verfasst werden. **0 % Java.** Eine `.java`-Datei irgendwo im
Repository ist ein Regelverstoß – es gibt keinen Fall, in dem eine neue Java-Datei richtig wäre.

„Nativ" heißt: **kein transliteriertes Java.** Kotlin-Code, der aussieht wie Java mit anderer
Syntax, erfüllt diesen Grundsatz nicht. Verbindlich:
...
| `x != null ? x : y` | `x ?: y` |
| `new Runnable() { … }` / anonyme Klasse | Lambda / SAM-Konvertierung, `Consumer<T>` als `{ t -> … }` |
...
| Nur-Daten-Klasse mit `equals`/`hashCode`/Gettern | `data class` |
| `new HashMap<>()` / `new ArrayList<>()` | `mutableMapOf()` / `mutableListOf()` |
| `List.of(...)` / `Map.of(...)` | `listOf(...)` / `mapOf(...)` |
| `StringBuilder` + Schleife für Text | String-Templates, `buildString`, `joinToString` |
| Javadoc `/** … */` mit `@param` | KDoc mit `[Referenz]`-Syntax |

**`!!` ist verboten**, wo eine Prüfung möglich ist. Erlaubt ist stattdessen `?.`, `?:`,
`requireNotNull(...)` mit Meldung, `checkNotNull(...)` oder ein `lateinit var` für Felder, die
garantiert vor dem ersten Zugriff gesetzt sind.

**Nullability wird nicht geraten.** Der Compiler läuft mit `-Xjsr305=strict`, damit Papers
`@NotNull`/`@Nullable` verbindlich sind. Wo eine Bukkit-API unannotiert ist (Plattformtyp), wird
die Nullability wie jede andere API-Frage **gegen die JAR geprüft** (siehe „Pflicht-Vorgehen"),
nicht angenommen.

### Rangfolge der beiden Grundsätze

Grundsatz 1 bestimmt die **API-Wahl**, Grundsatz 2 die **Sprachform**. Sie widersprechen sich
nicht – aber wo eine Kotlin-Bequemlichkeit eine Paper-Vorgabe unterlaufen würde, gewinnt Paper:

- **Keine `kotlinx-coroutines`.** Nebenläufigkeit läuft ausschließlich über Papers Scheduler
  (Vorgabe 3). Ein `suspend fun` auf dem Main-Thread-Tick ist kein Ersatz für
  `Bukkit.getRegionScheduler()`.
- **Keine Kotlin-Wrapper-Bibliotheken** für Bukkit/Paper. Die Paper-API wird direkt aufgerufen.
- `CompletableFuture` aus `teleportAsync` bleibt `CompletableFuture` – `.thenAccept { … }` ist
  bereits idiomatisch.

### Laufzeit-Abhängigkeit: kotlin-stdlib

Paper bringt die kotlin-stdlib **nicht** mit, und `paper-plugin.yml` hat kein `libraries:`-Feld
(verifiziert: `PluginMeta` besitzt keinen `getLibraries()`). Sie wird deshalb **in die JAR
gebündelt** – über eine eigene `bundled`-Konfiguration in `build.gradle.kts`, die `tasks.jar`
entpackt. Kein Shadow-Plugin nötig, es geht um genau eine Abhängigkeit ohne Paketumbenennung.

> ⚠️ **Papers `PluginLoader` + `MavenLibraryResolver` funktioniert hier nicht** – auch wenn die
> Landkarte ihn generell dem Shaden vorzieht. Die Loader-Klasse ist selbst Kotlin und wird vom
> `PaperSimplePluginClassLoader` geladen, der **ausschließlich die Plugin-JAR** sieht. Sie bräuchte
> die stdlib also bereits, um die stdlib nachladen zu können. Der Server bricht mit
> `NoClassDefFoundError: kotlin/jvm/internal/Intrinsics` in `PluginInitializerManager` ab, das
> Plugin wird gar nicht erst initialisiert. Am laufenden Server gemessen, nicht vermutet.
>
> Für **weitere** Bibliotheken (die nicht der Loader selbst braucht) bleibt der `PluginLoader` der
> richtige Weg – dann steht die stdlib ja schon in der JAR.
...
Gedächtnis sind regelmäßig falsch. Das gilt in Kotlin **doppelt**: Ob ein Rückgabetyp nullable ist,
entscheidet über `?.` oder direkten Zugriff, und geraten wird das nicht. Vor der Verwendung einer
API **immer** gegen die tatsächliche JAR prüfen:

Die JAR liegt nach dem ersten Gradle-Build im Cache — das ist der Pfad, der nach einem frischen
Clone verfügbar ist (ein Testserver gehört nicht ins Repository):
...
> Wer lokal einen Testserver betreibt, findet dieselbe JAR unter
> `Server/libraries/io/papermc/paper/paper-api/…`.
> Nullability steht in den Annotationen und ist nur mit `javap -v` sichtbar:
> `javap -v -cp $J org.bukkit.entity.Player | grep -B5 "Nullable"`.
...
1. `.\gradlew.bat build` läuft fehlerfrei durch.
2. **Null Warnungen** über den gesamten Quellcode. Der Build erzwingt das selbst: in
   `build.gradle.kts` steht `allWarningsAsErrors = true`, jede Deprecation- oder
   Removal-Warnung bricht den Build ab. Warnungen werden **behoben, nicht unterdrückt** –
   `@Suppress("DEPRECATION")` ist ein Regelverstoß, kein Workaround.
3. Grep-Gegenprobe ist leer:
   - **Paper (Grundsatz 1):** `§`, `ChatColor`, `BukkitRunnable`, `Bukkit.getScheduler()`,
     ...
     `GameRule.`, `ItemStack(`, `setPlayerListName(`.
   - **Kotlin (Grundsatz 2):** keine `*.java`-Datei im Repository, kein `!!`, kein
     `kotlinx.coroutines`, kein `@Suppress`, und kein parameterloser Getter
     (`grep -rE 'fun get[A-Z][A-Za-z]*\(\)' src/main/kotlin`) - der gehoert als Property
     geschrieben. Nachschlagemethoden **mit** Parameter (`getKills(uuid)`) sind ausdruecklich in
     Ordnung, ebenso Methoden mit Nebenwirkung oder wechselndem Ergebnis
     (`getRandomArenaLocation()`).

   > Die letzten Paper-Muster fängt der Compiler **nicht** zuverlässig ab – `setItemMeta` und der
   > `ItemStack`-Konstruktor sind gar nicht deprecated, sondern nur die schlechtere Wahl. Genau
...
Gebaut wird mit **Gradle** über den mitgelieferten Wrapper – eine Gradle-Installation im `PATH` ist
nicht nötig, eine Kotlin-Installation ebenfalls nicht (den Compiler zieht das Kotlin-Gradle-Plugin
selbst in den Gradle-Cache).
