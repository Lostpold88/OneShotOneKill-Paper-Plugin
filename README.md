...
- **☢ Nuke-Finale – so endet eine Runde**:
  ...
  - **Freigabemenü mit vierstelligem Code**: Rechtsklick öffnet ein Menü, in dem der Code steht und über Zifferntasten abgetippt werden muss. Darunter liegen **Abbrechen** und **Bestätigen**; bestätigen lässt sich erst, wenn vier Ziffern eingegeben sind, und ein falscher Code setzt die Eingabe zurück. Der Code wird bei jedem Öffnen neu gewürfelt, *Abbrechen* behält den Auslöser.
    ...
    ```
  - **Der Anflug (8 Sekunden)**: Ein sichtbarer Sprengkopf – ein dreifach vergrößertes, rot leuchtendes `BlockDisplay` – sinkt auf die Arenamitte zu, begleitet von Rauchfahne, Zielsäule vom Boden aus, Sirene und einem Sekunden-Countdown über dem Bildschirm. Er fällt **quadratisch beschleunigt**, wird also sichtbar schneller, je näher er kommt. Auf einer überdachten Map startet er unterhalb der Decke – sonst hinge er überm Dach und der ganze Anflug wäre für die Arena unsichtbar.
  - **Die Detonation**: weißer Blitz (`Particle.FLASH`, das zwingend ein `Color`-Datenobjekt braucht), `EXPLOSION_EMITTER`, `SONIC_BOOM` und `GUST_EMITTER_LARGE` übereinander, dazu **vier Tonspuren gleichzeitig** – Knall, Donner, Grollen und Warden-Kreischen – und für alle zwei Sekunden Blindheit. Der Blitz soll nicht nur zu sehen, sondern zu spüren sein.
  - **Die Druckwelle zerstört die Map wirklich.** Ein Ring läuft in gut vier Sekunden von der Mitte nach außen und ebnet ein, was er überrollt: Pro Spalte bleibt der **unterste feste Block** stehen und wird zu verkohltem Gestein (Blackstone, Basalt, Deepslate, Magma, Tuff), alles darüber verschwindet – auch das Dach der Standard-Map. Zurück bleibt eine schwarze Trümmerfläche unter offenem Himmel. Getroffene werden von der Welle umgeworfen.
      - **Ein Bruchteil bleibt stehen – zerstört.** Auf rund **18 % der Fläche** überleben ein paar Blocklagen als Ruine, ebenfalls verkohlt. Entschieden wird das **zellenweise** (4×4 Blöcke) über einen Hash der Koordinaten, nicht je Block: Ein Würfel pro Spalte ergäbe einzelne Pilzsäulen, das grobe Raster dagegen zusammenhängende Mauerreste – und der Hash braucht keinen gespeicherten Zustand.
      - Eingeebnet wird **nur innerhalb der Arena-Grenzen**: Die Lobby liegt außerhalb und muss stehen bleiben, sonst gäbe es nach der Runde keinen Ort mehr, an den die Spieler zurückkönnen.
      - Der unterste Block bleibt mit Absicht liegen – die Spieler stehen bis zum Gas noch darauf und fielen sonst ins Leere.
      - **Jeder veränderte Block wird vorher gesichert** (`BlockPosition` → `BlockData`) und beim nächsten `/osok start` wiederhergestellt. Ohne das wäre die Karte nach der ersten Runde dauerhaft ein Krater. Gesetzt wird ohne Physik (`setType(..., applyPhysics = false)`) – mit Physik lösen zehntausende Änderungen Nachbar-Updates, Sandfall und Lichtneuberechnungen aus und legen den Server lahm.
  - **Die Pilzwolke** steigt parallel dazu rund 20 Sekunden lang auf: ein Stiel aus Rauchringen, der sich nach oben verjüngt, und ab einem Viertel der Laufzeit ein Hut, der sich in vier Ringen öffnet, dazu Glut und Lava-Partikel im Fuß. Sie wird über `World#spawnParticle` gezeichnet – anders als beim Gas ist das hier richtig: Die Wolke ist ein Objekt in der Welt, das alle aus derselben Richtung sehen sollen.
  - **Sterben kann an alldem niemand**: Solange das Finale läuft, ist jeder Schadensweg abgeschaltet. Der Einschlag ist Inszenierung, getötet wird ausschließlich vom Gas.
  - **Dann tritt Giftgas aus** – dichte, giftgrüne Schwaden über der ganzen Karte. Die Optik entsteht auf zwei Wegen:
      - **Sechs Schichten um jeden Betrachter**: Fünfmal pro Sekunde bekommt jeder Spieler einen Stapel flacher, dichter Gaslagen gelegt – von knapp unter den Füßen bis rund acht Blöcke darüber, dazu einen extra dichten Bodenteppich. Ein einzelner hoher Quader verteilt seine Partikel gleichmäßig über die Höhe und wirkt dadurch dünn; gestapelte flache Lagen füllen den Raum wirklich. Gezeichnet wird über `Particle.DUST` mit `DustOptions` – nur damit lässt sich ein Partikel wirklich einfärben – mit **abwechselndem Grünton je Lage**, was der Wand Tiefe gibt, plus Rauch für Struktur.
      ...
      - **Fernwirkung** über ein überlappendes Raster aus `AreaEffectCloud`-Schwaden in derselben Farbe – **drei Ebenen übereinander** (Abstand 14, Radius 12), damit das Gas auch aus der Ferne in der Luft steht statt als Teppich am Boden zu liegen.
      - **Fallout**: Asche rieselt aus sechs Blöcken Höhe durch die Schwaden (`ASH` und `WHITE_ASH`), nach derselben Überlegung wie der Nebel – hoch angesetzt, weit gestreut, ein Aufruf pro Betrachter.
  ...
  - Die *Wirkung* hängt bewusst **nicht** an den Wolken (deren Trefferprüfung ist flach und endet an Wänden), sondern an einer eigenen Dosis-Buchführung: Jede Sekunde steigt die Dosis, Sicht und Tempo brechen ein, die Lebensanzeige sinkt sichtbar mit – und nach **12 Sekunden** erstickt der Spieler. Ab fünf Restsekunden wird es zusätzlich schwarz vor Augen. Dazu ein **Herzschlag, der mit der Dosis schneller und höher wird**, und das Knistern eines Geigerzählers, das sich zum Ende hin verdichtet.
  ...
  - Der nächste `/osok start` (oder `/osok stop`, oder ein Map-Wechsel) räumt alles ab: **die eingeebnete Arena wird Block für Block wiederhergestellt**, Gaswolken und Sprengkopf verschwinden, der Auslöser wird eingezogen – und **nur die Zuschauer, die das Finale selbst gesetzt hat**, kommen zurück ins Spiel. Wer freiwillig zuschaut, bleibt Zuschauer.
  - **Das Ende ist ein After-Action-Report.** Kein Feuerwerk, kein Regenbogen – nach einer Nuke, die die Arena eingeebnet hat, wäre Jahrmarkt der falsche Ton. Stattdessen: gut zwei Sekunden **Schwarzbild und Stille**, dann der harte Schnitt auf **MISSION ABGESCHLOSSEN** mit dem Namen des Siegers und einem tiefen Schlag – und danach läuft die Zusammenfassung **Zeile für Zeile** ein, jede mit eigenem Klick. Jede Zeile bekommt ihren Moment, statt als Block im Chat zu stehen.
  - Insgesamt dauert das Finale rund **25 Sekunden**: 8 Sekunden Anflug, 4 Sekunden Druckwelle, 12 Sekunden Gas – die Pilzwolke steht die ganze Zeit über der Arena.
...
- **🏆 Match-Manager & Dauer-Einstellung**:
  ...
  - Das Rundenende läuft als **After-Action-Report** im Stil eines Shooters (siehe Nuke-Finale) – ohne Musik, ohne Feuerwerk.
